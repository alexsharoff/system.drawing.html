...
namespace System.Drawing.Html
{
        ...
        : CssBox
    {
        ...
        public void FeedStyleSheet(string stylesheet)
        {
			...
			for (MatchCollection comments = Parser.Match(Parser.CssCommentsRegex, stylesheet); comments.Count > 0; comments = Parser.Match(Parser.CssCommentsRegex, stylesheet))
            {
            ...
            //MatchCollection atrules = Parser.Match(Parser.CssAtRulesRegex, stylesheet);

            for(MatchCollection atrules = Parser.Match(Parser.CssAtRulesRegex, stylesheet); atrules.Count > 0; atrules = Parser.Match(Parser.CssAtRulesRegex, stylesheet))
            {
                ...
                MatchCollection types = Parser.Match(Parser.CssMediaTypesRegex, atrule);
                ...
                if (types.Count == 1)
                {
                    ...
                    if (line.StartsWith("@media") && line.EndsWith("{"))
                    {
                        ...
                        for (int i = 0; i < media.Length; i++)
                        {
                            ...
                            MatchCollection insideBlocks = Parser.Match(Parser.CssBlocksRegex, atrule);
            ...
            MatchCollection blocks = Parser.Match(Parser.CssBlocksRegex, stylesheet);
        ...
        private void ParseDocument()
        {
            ...
            MatchCollection tags = Parser.Match(Parser.HtmlTagRegex, DocumentSource);
            CssBox curBox = root;
            int lastEnd = -1;
